// API client configuration for MyFoodMap
const API_BASE_URL = import.meta.env.PUBLIC_API_URL || 'http://localhost:3001/api';
const IS_DEMO_MODE = import.meta.env.PUBLIC_DEMO_MODE === 'true' || window.location.hostname.includes('netlify');

// Import demo client for Netlify deployment
let demoApiClient: any = null;
if (IS_DEMO_MODE) {
  import('./api-demo.js').then(module => {
    demoApiClient = module.apiClient;
  });
}
...
class ApiClient {
  ...
  private async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    // Use demo client if in demo mode
    if (IS_DEMO_MODE && demoApiClient) {
  ...
  private async routeToDemoClient<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    // Wait for demo client to load if not ready
    if (!demoApiClient) {
      const module = await import('./api-demo.js');
      demoApiClient = module.apiClient;
    }
